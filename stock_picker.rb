...
#   goo,mmm,adbe,ea,ba,ko,xom,gps,mcd, dis, crm, jnj =[]
#   array.each do |hash|
#     goo = hash[:goo]
#
# {:te }
...
#
#
# liste = [{ :GOO => 15, :MMM => 14, :ADBE => 12, :EA=> 13, :BA => 8, :KO => 10, :XOM => 20, :GPS => 7, :MCD => 11, DIS => 15, :CRM => 6, :JNJ => 10 }
...
def trader_du_dimanche(liste=[1,3,6,9,15,8,6,1,10])

  indexAchat,indexVente,ratio = 0, 1,0
  i = 0
  while (i<liste.size-2)
    achat = liste[i]
    j=1
    while(j<liste.size-2)
      pas = liste[j]-achat
      if ratio < pas
        ratio = pas
        indexVente = j
